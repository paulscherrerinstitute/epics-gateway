class StatsBarGraph
{
    static DrawStats(): void
    {
        ...
        if (!LogsPage.Stats || !LogsPage.Stats.Logs || !LogsPage.Stats.Logs.length)
            return;

        var end = LogsPage.Stats.Logs[LogsPage.Stats.Logs.length - 1].Date;

        //var w = width / 145;
        var w = width / LogsPage.Stats.Logs.length;
        var b = height * 2 / 3;
        var bn = height - b;

        var maxValue = 1;
        for (var i = 0; i < LogsPage.Stats.Logs.length; i++)
            maxValue = Math.max(maxValue, LogsPage.Stats.Logs[i].Value);
        var maxErrorValue = 1;
        for (var i = 0; i < LogsPage.Stats.Errors.length; i++)
            maxErrorValue = Math.max(maxErrorValue, LogsPage.Stats.Errors[i].Value);
        var maxSearchValue = 1;
        for (var i = 0; i < LogsPage.Stats.Searches.length; i++)
            maxSearchValue = Math.max(maxSearchValue, LogsPage.Stats.Searches[i].Value);
        ...
        var dts = Utils.ShortGWDateFormat(new Date(end.getTime() - end.getTimezoneOffset() * 60000 - LogsPage.Stats.Logs.length / 2 * 10 * 60 * 1000));
        var tw = ctx.measureText(dts).width;
        ctx.fillStyle = "rgba(255,255,255,0.7)";
        ...
        var dts = Utils.ShortGWDateFormat(new Date(end.getTime() - end.getTimezoneOffset() * 60000 - LogsPage.Stats.Logs.length * 10 * 60 * 1000));
        var tw = ctx.measureText(dts).width;
        ctx.fillStyle = "rgba(255,255,255,0.7)";
    ...
    {
        if (!LogsPage.Stats || !LogsPage.Stats[logType])
            return 0;
        var s: LogStat[] = LogsPage.Stats[logType];
    ...
    static TimeLineMouse(evt: JQueryMouseEventObject): void
    {
        ...
        if (tx < 0)
            tx = 0;
        if (tx >= LogsPage.Stats.Logs.length)
            tx = LogsPage.Stats.Logs.length - 1;
        //console.log(tx);
        ...
        Main.CurrentTime = new Date(LogsPage.Stats.Logs[LogsPage.Stats.Logs.length - 1].Date.getTime() - tx * 10 * 60 * 1000);
