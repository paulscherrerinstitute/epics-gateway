...
namespace GatewayDebugWindow
{
    ...
    public partial class MainWindow : Window
    {
        ...
        readonly List<Channel<string>> channels = new List<Channel<string>>();
        ...
        void ContextNewName(DebugContext ctx, string name)
        {
            this.Dispatcher.BeginInvoke((Action)delegate
            {
                ...
                epicsClient = new CAClient();
                //epicsClient.Configuration.SearchAddress = (string)Properties.Settings.Default["StoredGateway"];

                foreach (var i in channelNames)
                {
                    var channel = epicsClient.CreateChannel<string>(name + i);
                    channels.Add(channel);
                    channel.MonitorChanged += new ChannelValueDelegate<string>(ChannelMonitorChanged);
                }
            });
        }

        void ChannelMonitorChanged(Channel<string> sender, string newValue)
        {
            this.Dispatcher.BeginInvoke((Action)delegate
            {
